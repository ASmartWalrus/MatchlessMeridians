...
impl KungFu {
    pub fn check_overlap(self: &Self, other: &Self, i: i32) -> bool {
        return ((other.acupoint_bits >> ((other.length as i32 - self.length as i32 - i) * 2))
            & !(!0u64 << (self.length * 2)))
            == (self.acupoint_bits & !(!0u64 << ((self.length as i32 + i) * 2)));
    }
...
impl From<u64> for KungFu {
    fn from(meridian_bits: u64) -> Self {
        KungFu {
            acupoint_bits: meridian_bits,
            length: ((u64::BITS - meridian_bits.leading_zeros() + 1) / 2) as u32,
        }
    }
}
