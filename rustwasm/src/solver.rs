    ...
    pub fn build_soln(
        ...
        for (i, kf_idx) in solution_kf_idxs.iter().enumerate() {
            if i == 0 {
                ...
                let unoverlap_idx = (kfs[prev_kf_idx].length as i32
                    + memo[&kfs[prev_kf_idx].acupoint_bits][&kfs[*kf_idx].acupoint_bits])
                    as usize;
                solution
                    .mstring
                    .push_str(&Into::<String>::into(kfs[*kf_idx])[unoverlap_idx..]);
            ...
            if *kf_loc == usize::MAX {
                ...
                for other_idx in solution_kf_idxs.iter() {
                    ...
                    if overlap_pt >= 0 {
                        ...
                        solution.locations[kf_idx] =
                            solution.locations[*other_idx] + overlap_pt as usize;
                        break;
                    }
                }
                // Mark merge
...
impl Solver {
        ...
        for kf in self.kfs.iter() {
            if let None = self.memo.get(&kf.acupoint_bits) {
                ...
                for other_kf in self.kfs.iter() {
                    if kf.acupoint_bits != other_kf.acupoint_bits
                        && kf_map.get(&other_kf.acupoint_bits).is_none()
                    {
    ...
    fn filter_mergables(&mut self) {
        ...
        self.filtered_kf_idxs
            .dedup_by_key(|v| self.kfs[*v].acupoint_bits);

        // check against every existing acupoints signature
        // Filter out both blanks and mergables
        self.filtered_kf_idxs = self
            .filtered_kf_idxs
            .iter()
            .filter_map(|i| -> Option<usize> {
                ...
                if self
                    .memo
                    .get(&self.kfs[*i].acupoint_bits)
                    .and_then(|kf_map| kf_map.iter().find(|(_, v)| **v >= 0))
                    .is_none()
                {
                    Some(*i)
                } else {
    ...
    fn greedy_solve(&mut self) {
        ...
        let mut kf_groups: Vec<Vec<usize>> =
            self.filtered_kf_idxs.iter().map(|i| vec![*i]).collect();
            ...
            for (i, kfg_1) in kf_groups.iter().enumerate() {
                ...
                for (j, kfg_2) in kf_groups.iter().enumerate() {
                    if i != j {
                        ...
                        let len_reduct = kf_1.length as i32
                            + self.memo[&kf_1.acupoint_bits][&kf_2.acupoint_bits];
        ...
        while self.p[0] < self.greedy_kf_idxs.len() && runs < max_runs {
            ...
            for i in p_chg_idx..kf_perm.len() {
                if i == 0 {
                    ...
                    let overlap =
                        self.memo[&kf_perm[i - 1].acupoint_bits][&kf_perm[i].acupoint_bits];
                    self.p_lens[i] = self.p_lens[i - 1]
                        - (kf_perm[i - 1].length as i32 + overlap) as u32
                        + kf_perm[i].length;
                }
